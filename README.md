...
Los utilizamos para abstraer cada funcionalidad de nuestra aplicacion en varios componentes con responsabilidades unicas. Cada componente esta compuesto de 4 archivos:
...
Importar el servicio en el componente a ser utilizado
import { StoreService } from '../../services/store.service';
Para incluir a este servicio dentro del componente, vamos a crear algo llamado Inyeccion de dependencias
...
Crear el servicion encargado de realizar las peticiones HTTP.
Importar el Modulo HttpClientModule en el app.module de nuestra aplicacion.
Agregar el HttpClientModule a la seccion de imports
Vamos a nuestro servicio e importamos el SERVICIO HttpClient
Como el HttpClient es un servicio, debemos de declararlo en el constructor, como cualquier otro servicio
Realizamos la peticion a nuestra url con ayuda de HttpClient.get
...
Debemos ir a nuestro componente y crear la Inyeccion dependencias para poder utilizar el servicio.
    import { StoreService } from '../../services/store.service';
...
Como una peticion Http es una operacion asincrona, no podemos hacerla desde el constructor. Recordemos que un lugar apropiado para utilizar funcionalidades asincronas es en el ngOnInit
    import { StoreService } from '../../services/store.service';
    import { ProductsService } from '../../services/products.service';
...
Angular maneja un formato por defecto para los resultados asincronos; un Observable. Podemos acceder a los datos de este Observable utilizando .subscribe
    ngOnInit(): void {
        this.productsService.getAllProducts()
        .subscribe(data => {
        this.products = data;
...
Nuestra variable date_var constituye la entrada del pipe.
<p> Fecha: {{ date_var }}</p>
El simbolo | es el que utilizamos para 'decir' que vamos a utilizar un Pipe
<p> Fecha: {{ date_var | }}</p>
Luego le sigue el nombre del Pipe.
<p> Fecha: {{ date_var | date }}</p>
Y por ultimo tenemos el formato que queremos darle a nuestra salida
...
Nos suscribimos desde el ngOnInit o desde el constructor
    import { StoreService } from '../../services/store.service'
...
Pasos
Configurar el linter ng add @angular-eslint/schematics

Correr el linter ng lint
